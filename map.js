/****
 * 部分功能未测试
 * 关键字搜索后传入后台的数据为baidu的坐标，可能经后台转换后有偏移
 * 赞美猴菊
 * **/
...
//转换完的百度皮肤
var pGoStyle=[{"featureType":"manmade","elementType":"geometry.fill","stylers":{"color":"#37bda2"}},{"featureType":"land","elementType":"geometry.fill","stylers":{"color":"#a1f199"}},{"featureType":"green","elementType":"geometry.fill","stylers":{"color":"#37bda2"}},{"featureType":"poi.attraction","elementType":"geometry.fill","stylers":{"visibility":"on"}},{"featureType":"poi.business","elementType":"geometry.fill","stylers":{"color":"#e4dfd9"}},{"featureType":"poi.business","elementType":"labels.icon","stylers":{"visibility":"off"}},{"featureType":"poi.park","elementType":"geometry.fill","stylers":{"color":"#37bda2"}},{"featureType":"road","elementType":"geometry.fill","stylers":{"color":"#84b09e"}},{"featureType":"road","elementType":"geometry.stroke","stylers":{"color":"#fafeb8","weight":"1.25"}},{"featureType":"highway","elementType":"labels.icon","stylers":{"visibility":"off"}},{"featureType":"water","elementType":"geometry.fill","stylers":{"color":"#5ddad6"}}];
...
function initMap() {
	...
	var startPoint = new BMap.Point(center_lng,center_lat);
	map.centerAndZoom(startPoint,16);
	map.addOverlay(new BMap.Marker(startPoint));
	map.addControl(new BMap.MapTypeControl({mapTypes: [BMAP_NORMAL_MAP,BMAP_HYBRID_MAP],anchor: BMAP_ANCHOR_TOP_LEFT}));//添加地图模式切换
	map.enableScrollWheelZoom(true);//允许鼠标滚轮缩放	
	
    marker = new BMap.Marker(new BMap.Point(center_lng,center_lat));
...
function initSidebar() {
	...
	function setPlace(){
		...
		function myFun(){
			...
			$.post("next_loc?lat=" + pp.lat + "&lon=" + pp.lng, {}).done(function (data) {
...
function setupPokemonMarker(item) {
	var point = new BMap.Point(item.longitude,item.latitude);//建立点
	var newIcon = new BMap.Icon('static/icons/' + item.pokemon_id + '.png',new BMap.Size(30,30));//创建图标
	var marker = new BMap.Marker(point,{icon:newIcon});//创建覆盖物

	
	var infoWindow = new BMap.InfoWindow(pokemonLabel(item.pokemon_name, item.disappear_time, item.pokemon_id, item.latitude, item.longitude));//创建信息窗
	map.addOverlay(marker);//覆盖物加入地图
...
function setupGymMarker(item) {
	...
	var newIcon = new BMap.Icon('static/forts/' + gym_types[item.team_id] + '.png',new BMap.Size(30,30));
	var marker = new BMap.Marker(point,{icon:newIcon});

	var infoWindow = new BMap.InfoWindow(gymLabel(gym_types[item.team_id], item.team_id, item.gym_points));
    addListeners(marker,infoWindow);
...
function setupPokestopMarker(item) {
    ...
    var newIcon = new BMap.Icon('static/forts/' + imagename + '.png',new BMap.Size(30,30));
    var maker = new BMap.Marker(point,{icon:newIcon});

	var infoWindow = new BMap.InfoWindow(pokestopLabel(!!item.lure_expiration, item.last_modified, item.active_pokemon_id, item.latitude, item.longitude));
	addListeners(marker,infoWindow);
...
function setupScannedMarker(item) {
    ...
    var circle = new BMap.circle(point,500,{strokeColor:getColorByDate(item.last_modified),strokeWeight:1})

	var maker = new BMap.Maker(circle);
	var infoWindow = new BMap.InfoWindow(scannedLabel(item.last_modified));

    addListeners(marker,info);
...
//删除点,将marker传入使用removeOverlay删除该marker,全部删除使用map.clearOverlays()
...
function clearStaleMarkers() {
    $.each(map_pokemons, function(key, value) {
        ...
        if (map_pokemons[key]['disappear_time'] < new Date().getTime() ||
			...
			deletePoint(map_pokemons[key].marker);
        }
    });
    ...
    $.each(map_scanned, function(key, value) {
        ...
        if (map_scanned[key]['last_modified'] < (new Date().getTime() - 15 * 60 * 1000)) {
			deletePoint(map_scanned[key].marker);
        }
    });
};
        ...
        $.each(result.gyms, function(i, item){
            ...
            if (item.gym_id in map_gyms) {
                ...
                if (map_gyms[item.gym_id].team_id != item.team_id) {
                	...
                	map_gyms[item.gym_id].marker.infoWindow = new BMap.InfoWindow(gymLabel(gym_types[item.team_id], item.team_id, item.gym_points));
                }
            }
